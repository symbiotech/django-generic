    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in self.fields.keys():
            ...
            if isinstance(model_field, models.ManyToManyField):
                ...
                self.fields['m2m_remove_%s'%(field_name,)] = copy(self.fields[field_name])
                self.fields['m2m_remove_%s'%(field_name,)].label = _("Remove %s"%(field_name,))
                self.fields['m2m_remove_%s'%(field_name,)].update_checkbox = BoundField(
                    self,
                    forms.BooleanField(required=False),
                    'updating-'+'m2m_remove_%s'%(field_name,)
                )

                self.fields['m2m_add_%s'%(field_name,)] = copy(self.fields[field_name])
                self.fields['m2m_add_%s'%(field_name,)].label = _("Add %s"%(field_name,))
                self.fields['m2m_add_%s'%(field_name,)].update_checkbox = BoundField(
                    self,
                    forms.BooleanField(required=False),
                    'updating-'+'m2m_add_%s'%(field_name,)
                )
